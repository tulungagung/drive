const authConfig = {
  ...
  siteIcon: "https:////cdn.jsdelivr.net/gh/tulungagung/drive@1.2.7/assets/images/favicon-x.png", // or favicon-x-light.png
  version: "1.2.7",
...
async function handleId2Path(request, gd) {
  ...
  let form = await request.formData();
  let path = await gd.findPathById(
    decryptAES(form.get("id"), authConfig.crypt_secret)
  );
...
class googleDrive {
  ...
  async _file(path) {
    ...
    if (/\(dupID: \d+\)/.test(name.substring(name.search(/\(dupID: \d+\)/)))) {
      dupID = name
        ...
        .substring(name.search(/\(dupID: \d+/) + 8);
    ...
    if (
      obj.files &&
      obj.files[0] &&
      obj.files[0].mimeType == "application/vnd.google-apps.shortcut"
    ) {
  ...
  async search(origin_keyword, page_token = null, page_index = 0) {
    ...
    if (is_sub_folder) {
      ...
      if (authConfig.folder_list_url) {
        ...
        if (target_folder.length > 1) {
          ...
          (huge_folder_list = true), (params.pageSize = 200);
          params.fields =
            "nextPageToken, files(id, name, mimeType, size , modifiedTime, parents)";
        } else {
          search_q_ids =
            "('" + target_folder[0].join("' in parents or '") + "' in parents)";
    ...
    while (
      response.status != 200 ||
      (res_obj.nextPageToken && res_obj.files.length == 0)
    ) {
      if (response.status != 200) {
    ...
    if (huge_folder_list) {
        ...
        res_obj.files.map(function (item) {
          if (item.parents) {
            if (search_q_ids.includes(item.parents[0])) {
              item.parents[0] = encryptAES(
                item.parents[0],
                authConfig.crypt_secret
              );
      ...
      if (repeat_until_fetch_enough) {
        while (
          response.status != 200 ||
          (res_obj.nextPageToken && temp_files.length < 10)
        ) {
          if (response.status != 200) {
            param.pageToken = null;
          } else {
  ...
  async _findDirId(parent, name) {
    ...
    if (
      obj.files[0].mimeType == "application/vnd.google-apps.shortcut" &&
      obj.files[0].shortcutDetails.targetMimeType ==
